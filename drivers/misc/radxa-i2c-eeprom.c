...
static struct eeprom_info einfo;

static uchar eeprom_wp_buff[CONFIG_EEPROM_HATS_SIZE_MAX];

static struct eeprom_hats_header eeprom_hats_header_default = {
	.signature = EEPROM_HATS_SIG,
	.version = FORMAT_VERSION,
	.numatoms = 2,
	.eeplen = sizeof(struct eeprom_hats_header) +
		  sizeof(struct eeprom_atom1) +
		  sizeof(struct eeprom_atom4_v1)
};

static struct eeprom_atom1 eeprom_atom1_default = {
	.header = {
		.type = HATS_ATOM_VENDOR,
		.count = 1,
		.dlen = sizeof(struct eeprom_atom1_data) + sizeof(u16)
	},
	.data = {
		.uuid = {0},
		.pid = 0,
		.pver = 0,
		.vslen = CONFIG_EEPROM_ATOM1_VSTR_SIZE,
		.pslen = CONFIG_EEPROM_ATOM1_PSTR_SIZE,
		.vstr = CONFIG_EEPROM_ATOM1_VSTR,
		.pstr = CONFIG_EEPROM_ATOM1_PSTR
	}
};

static struct eeprom_atom4_v1 eeprom_atom4_v1_default = {
	.header = {
		.type = HATS_ATOM_CUSTOM,
		.count = 2,
		.dlen = sizeof(struct eeprom_atom4_v1_data) + sizeof(u16)
	},
	.data = {
		.version = FORMAT_VERSION,
...
//static u8 default_mac[MAC_ADDR_BYTES] = DEFAULT_MAC;
 ...
 */
static void show_eeprom(struct eeprom_info *einfo)
{
	printf("\n--------EEPROM INFO--------\n");
	printf("Vendor : %s\n", einfo->vstr);
...
static int prog_eeprom(uint8_t *buf, unsigned int size)
{
	unsigned int i;
	void *p;
	uchar tmp_buff[CONFIG_EEPROM_HATS_SIZE_MAX];
	struct udevice *dev;
...
static void set_mac_address(char *string)
{
	...
	atom4 = (struct eeprom_hats_atom_header *)
		hats_get_atom((struct eeprom_hats_header *)eeprom_wp_buff,
			      HATS_ATOM_CUSTOM);
	if (strncasecmp(OUI_PREFIX, string,
			strlen(OUI_PREFIX))) {
		printf("The MAC address doesn't match ROCK OUI %s\n",
		       OUI_PREFIX);
		return;
	}
...
/**
 * set_pcb_revision() - stores a ROCK PCB revision into the local EEPROM copy
 *
...
/**
 * set_bom_revision() - stores a ROCK BOM revision into the local EEPROM copy
 *
...
/**
 * set_product_id() - stores a ROCK product ID into the local EEPROM copy
 *
...
static void init_local_copy(uchar *buff)
{
	...
	struct eeprom_hats_atom_header *atom4_v1 = (void *)atom1 +
					sizeof(struct eeprom_atom1);
	memcpy((void *)hats, (void *)&eeprom_hats_header_default,
	       sizeof(struct eeprom_hats_header));
	memcpy((void *)atom1, (void *)&eeprom_atom1_default,
	       sizeof(struct eeprom_atom1));
	memcpy((void *)atom4_v1, (void *)&eeprom_atom4_v1_default,
	       sizeof(struct eeprom_atom4_v1));
	update_crc(atom1);
	update_crc(atom4_v1);
}
...
static int print_usage(void)
{
	...
	"mac pcb_revision <?>\n"
	"    - stores a ROCK PCB revision into the local EEPROM copy\n"
	"mac bom_revision <A>\n"
	"    - stores a ROCK BOM revision into the local EEPROM copy\n"
	"mac product_id <VF7100A1-2150-D008E000-xxxxxxxx>\n"
	"    - stores a ROCK product ID into the local EEPROM copy\n");
	return 0;
}
...
int do_mac(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (argc != 3)
		return print_usage();
	if (is_match_magic((char*)eeprom_wp_buff)) {
...
int mac_read_from_eeprom(void)
{
	...
	if (read_eeprom(eeprom_wp_buff)) 
		return 0;
	// 1, setup ethaddr env
	eth_env_set_enetaddr("eth1addr",einfo.mac_addr);
